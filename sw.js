// ============================================================
// AUMATIQ Finance Tracker — Service Worker (sw.js)
// Version: aumatiq-finance-v1
//
// Cache update rule:
//   index.html বা কোনো file update করলে CACHE_NAME version বাড়াও:
//   'aumatiq-finance-v1' → 'aumatiq-finance-v2'
//   পুরনো cache auto-delete হবে, নতুন cache নেবে।
// ============================================================

var CACHE_NAME = 'aumatiq-finance-v2';
...
self.addEventListener('install', function(event) {
  console.log('[AUMATIQ SW] Installing v1...');
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(function(cache) {
        console.log('[AUMATIQ SW] Caching app shell');
      ...
      .catch(function(err) {
        console.error('[AUMATIQ SW] Install failed:', err);
      })
  );
});
...
self.addEventListener('activate', function(event) {
  console.log('[AUMATIQ SW] Activating...');
  event.waitUntil(
    caches.keys()
        ...
        return Promise.all(
          cacheNames.map(function(cacheName) {
            if (cacheName !== CACHE_NAME) {
              console.log('[AUMATIQ SW] Deleting old cache:', cacheName);
...
self.addEventListener('fetch', function(event) {
  ...
  // Google Apps Script API → Network First (data always fresh)
